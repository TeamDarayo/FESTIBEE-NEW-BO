...
import type { TimeTableDetailRes, StageRes } from "../api/performance-api";
...
import { StageManagerPanel } from "./stage-manager-panel";
import { useStageList } from "../hooks/use-stage-mutations";
...
export function CastingEditor({
  ...
  // Load stages for this performance (홀 = 공연 스테이지)
  const { data: stagesData } = useStageList(performanceId);
  const stages: StageRes[] = (stagesData ?? []) as StageRes[];
  ...
  return (
    <div className="p-4">
      ...
      <StageManagerPanel performanceId={performanceId} stages={stages} />

      <CastingSpreadsheet rows={rows} dispatch={wrappedDispatch} stages={stages} />
    </div>
  );
}
